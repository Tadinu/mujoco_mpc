...
mjModel* LoadModel(const char* file, mjapp::Simulate& sim) {
  ...
  auto load_interval = mjapp::Simulate::Clock::now() - load_start;
...
void PhysicsLoop(mjapp::Simulate& sim) {
  // cpu-sim syncronization point
  std::chrono::time_point<mjapp::Simulate::Clock> syncCPU;
  mjtNum syncSim = 0;
...

    {
      ...
      if (m) {
        ...
        if (sim.run) {
          ...
          const auto startCPU = mjapp::Simulate::Clock::now();
          ...
          // in-sync: step until ahead of cpu
            ...
            while (Seconds((d->time - syncSim) * slowdown) < mjapp::Simulate::Clock::now() - syncCPU &&
                   mjapp::Simulate::Clock::now() - startCPU < Seconds(refreshTime)) {
