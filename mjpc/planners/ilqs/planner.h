  ...
  void ActionFromPolicy(double* action_, const double* state_,
    ...
    if (use_previous_) {
      if (previous_active_policy == kSampling) {
        ...
        sampling.ActionFromPolicy(action_, state_, time_, true);
        ...
        if (active_policy == kSampling) {
          ...
          ilqg.ActionFromPolicy(action_, state_, time_, false);
        } else {
          // Most recent planner step updated iLQG, so the previous policy is
          // iLQG's previous policy.
          ilqg.ActionFromPolicy(action_, state_, time_, true);
        }
      }
    } else {  // use current policy
      if (active_policy == kSampling) {
        sampling.ActionFromPolicy(action_, state_, time_, false);
      } else {
        ilqg.ActionFromPolicy(action_, state_, time_, false);
      }
    }
