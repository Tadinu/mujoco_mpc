...
IdtoMPC::IdtoMPC(const Diagram<double>* diagram, const MultibodyPlant<double>* plant,
    ...
    : time_step_(plant->time_step()),
      ...
      warm_start_(optimizer_.num_steps(), optimizer_.diagram(), optimizer_.plant(),
                  optimizer_.num_equality_constraints(), warm_start_solution.q, params.Delta0) {
  // Abstract-valued discrete state stores an optimal trajectory
  IdtoStoredTrajectory initial_guess_traj;
  StoreOptimizerSolution(warm_start_solution, 0.0, &initial_guess_traj);
  stored_trajectory_ = this->DeclareAbstractState(drake::Value(initial_guess_traj));
  this->DeclarePeriodicUnrestrictedUpdateEvent(replan_period, 0, &IdtoMPC::UpdateAbstractState);

  // Input port recieves state estimates
  state_input_port_ =
      this->DeclareVectorInputPort("state_estimate", BasicVector<double>(nq_ + nv_)).get_index();

  // Output port sends the optimal trajectory
  trajectory_output_port_ =
      this->DeclareStateOutputPort("optimal_trajectory", stored_trajectory_).get_index();
}

EventStatus IdtoMPC::UpdateAbstractState(const Context<double>& context, State<double>* state) const {
  ...
  auto& stored_trajectory = state->get_mutable_abstract_state<IdtoStoredTrajectory>(stored_trajectory_);
...
void IdtoMPC::UpdateInitialGuess(const IdtoStoredTrajectory& stored_trajectory, const double current_time,
...
void IdtoMPC::StoreOptimizerSolution(const TrajectoryOptimizerSolution<double>& solution,
                                     const double start_time, IdtoStoredTrajectory* stored_trajectory) const {
  ...
  stored_trajectory->q =
      PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(time_steps, q_knots);
  stored_trajectory->v =
      PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(time_steps, v_knots);
  stored_trajectory->u =
      PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(time_steps, u_knots);
}

IdtoInterpolator::IdtoInterpolator(const int nq, const int nv, const int nu) : nq_(nq), nv_(nv), nu_(nu) {
  // Input port recieves a IdtoStoredTrajectory
  trajectory_input_port_ =
      this->DeclareAbstractInputPort("trajectory", drake::Value(IdtoStoredTrajectory())).get_index();

  // First output port sends interpolated state values x(t)
  state_output_port_ =
      this->DeclareVectorOutputPort("state", BasicVector<double>(nq_ + nv_), &IdtoInterpolator::SendState)
          .get_index();

  // First output port sends interpolated control values u(t)
  control_output_port_ =
      this->DeclareVectorOutputPort("control", BasicVector<double>(nu_), &IdtoInterpolator::SendControl)
          .get_index();
}

void IdtoInterpolator::SendState(const Context<double>& context, BasicVector<double>* output) const {
  const IdtoStoredTrajectory& traj =
      EvalAbstractInput(context, trajectory_input_port_)->get_value<IdtoStoredTrajectory>();
...
void IdtoInterpolator::SendControl(const Context<double>& context, BasicVector<double>* output) const {
  const IdtoStoredTrajectory& traj =
      EvalAbstractInput(context, trajectory_input_port_)->get_value<IdtoStoredTrajectory>();
