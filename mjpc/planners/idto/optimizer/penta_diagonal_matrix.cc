...
template <typename T>
PentaDiagonalMatrix<T>::PentaDiagonalMatrix(int num_blocks, int block_size, bool is_symmetric)
...
template <typename T>
PentaDiagonalMatrix<T>::PentaDiagonalMatrix(std::vector<MatrixX<T>> A, std::vector<MatrixX<T>> B,
...
template <typename T>
PentaDiagonalMatrix<T> PentaDiagonalMatrix<T>::MakeSymmetricFromLowerDense(const MatrixX<T>& M,
  ...
  for (int i = 0; i < num_blocks; ++i) {
    if (i >= 2) A[i] = M.block(i * block_size, (i - 2) * block_size, block_size, block_size);
    if (i >= 1) B[i] = M.block(i * block_size, (i - 1) * block_size, block_size, block_size);
...
MatrixX<T> PentaDiagonalMatrix<T>::MakeDense() const {
  ...
  for (int i = 0; i < num_blocks; ++i) {
    if (i >= 2) M.block(i * block_size(), (i - 2) * block_size(), block_size(), block_size()) = A_[i];
    if (i >= 1) M.block(i * block_size(), (i - 1) * block_size(), block_size(), block_size()) = B_[i];
...
template <typename T>
bool PentaDiagonalMatrix<T>::VerifyAllBlocksOfSameSize(const std::vector<MatrixX<T>>& X, int size) {
...
template <typename T>
void PentaDiagonalMatrix<T>::MultiplyBy(const VectorX<T>& v, VectorX<T>* result) const {
...
void PentaDiagonalMatrix<T>::ScaleByDiagonal(const VectorX<T>& scale_factor) {
  ...
  for (int i = 0; i < block_rows(); ++i) {
    // Diagonal blocks are always present
    C_[i] =
        scale_factor.segment(i * bs, bs).asDiagonal() * C_[i] * scale_factor.segment(i * bs, bs).asDiagonal();
