...
#include "mjpc/planners/idto/optimizer/penta_diagonal_solver.h"
#include "mjpc/planners/idto/utils/profiler.h"
...
template <typename T>
TrajectoryOptimizer<T>::TrajectoryOptimizer(const Diagram<T>* diagram, const MultibodyPlant<T>* plant,
                                            const ProblemDefinition& prob, const SolverParameters& params)
  ...
  if constexpr (std::is_same_v<T, double>) {
    if ((params_.gradients_method == GradientsMethod::kAutoDiff) || params_.exact_hessian) {
      ...
      optimizer_ad_ =
          std::make_unique<TrajectoryOptimizer<AutoDiffXd>>(diagram_ad_.get(), plant_ad_, prob, params_ad);
      // TODO(vincekurtz): move state's destructor and possible other
      // implementation to the source?
      state_ad_ =
          std::unique_ptr<TrajectoryOptimizerState<AutoDiffXd>>(new TrajectoryOptimizerState<AutoDiffXd>(
              num_steps(), *diagram_ad_, *plant_ad_, num_equality_constraints()));
    }
  } else {
    if (params_.gradients_method == GradientsMethod::kAutoDiff) {
  ...
  if ((params_.gradients_method != GradientsMethod::kForwardDifferences) && (params_.num_threads != 1)) {
    drake::log()->warn(
...
template <typename T>
T TrajectoryOptimizer<T>::CalcCost(const std::vector<VectorX<T>>& q, const std::vector<VectorX<T>>& v,
...
template <typename T>
void TrajectoryOptimizer<T>::CalcAccelerations(const std::vector<VectorX<T>>& v,
...
template <typename T>
void TrajectoryOptimizer<T>::CalcInverseDynamicsSingleTimeStep(const Context<T>& context, const VectorX<T>& a,
...
template <typename T>
void TrajectoryOptimizer<T>::CalcContactForceContribution(const Context<T>& context,
  ...
  const drake::geometry::QueryObject<T>& query_object =
      plant().get_geometry_query_input_port().template Eval<drake::geometry::QueryObject<T>>(context);
  ...
  for (const SignedDistancePair<T>& pair : signed_distance_pairs) {
    ...
    const Body<T>& bodyA = *(plant().GetBodyFromFrameId(inspector.GetFrameId(geometryA_id)));
    const Body<T>& bodyB = *(plant().GetBodyFromFrameId(inspector.GetFrameId(geometryB_id)));

    // Body poses in world.
    const drake::math::RigidTransform<T>& X_WA = plant().EvalBodyPoseInWorld(context, bodyA);
    const drake::math::RigidTransform<T>& X_WB = plant().EvalBodyPoseInWorld(context, bodyB);

    // Geometry poses in body frames.
    const drake::math::RigidTransform<T> X_AGa = inspector.GetPoseInFrame(geometryA_id).template cast<T>();
    const drake::math::RigidTransform<T> X_BGb = inspector.GetPoseInFrame(geometryB_id).template cast<T>();
    ...
    // Velocities.
    const SpatialVelocity<T>& V_WA = plant().EvalBodySpatialVelocityInWorld(context, bodyA);
    const SpatialVelocity<T>& V_WB = plant().EvalBodySpatialVelocityInWorld(context, bodyB);
    const SpatialVelocity<T> V_WAc = V_WA.Shift(p_AC_W);
    const SpatialVelocity<T> V_WBc = V_WB.Shift(p_BC_W);

    // Relative contact velocity.
    const drake::Vector3<T> v_AcBc_W = V_WBc.translational() - V_WAc.translational();
    ...
    const drake::Vector3<T> that_regularized = -vt / sqrt(vs * vs + vt.squaredNorm());
...
template <typename T>
void TrajectoryOptimizer<T>::CalcInverseDynamicsPartials(const TrajectoryOptimizerState<T>& state,
...
void TrajectoryOptimizer<T>::CalcInverseDynamicsPartialsFiniteDiff(
    const TrajectoryOptimizerState<T>& state, InverseDynamicsPartials<T>* id_partials) const {
  using std::abs;
  using std::max;
...
void TrajectoryOptimizer<T>::CalcInverseDynamicsPartialsCentralDiff(
  ...
  for (int t = 1; t <= num_steps(); ++t) {
    ...
    CalcInverseDynamicsPartialsWrtQtCentralDiff(t, state, dtaum_dqt, dtaut_dqt, dtaup_dqt);
  }
}

template <typename T>
void TrajectoryOptimizer<T>::CalcInverseDynamicsPartialsWrtQtCentralDiff(
    int t, const TrajectoryOptimizerState<T>& state, MatrixX<T>* dtaum_dqt, MatrixX<T>* dtaut_dqt,
    MatrixX<T>* dtaup_dqt) const {
  using std::abs;
  using std::max;
  ...
  bool fourth_order = (params_.gradients_method == GradientsMethod::kCentralDifferences4);
  ...
  for (int i = 0; i < plant().num_positions(); ++i) {
    ...
    if (t > 0) {
      ...
      CalcInverseDynamicsSingleTimeStep(*context_, a_em_tm, &workspace, &tau_em_tm);
      plant().SetPositions(context_, q_ep_t);
      plant().SetVelocities(context_, v_ep_t);
      CalcInverseDynamicsSingleTimeStep(*context_, a_ep_tm, &workspace, &tau_ep_tm);
      if (fourth_order) {
        plant().SetPositions(context_, q_emm_t);
        plant().SetVelocities(context_, v_emm_t);
        CalcInverseDynamicsSingleTimeStep(*context_, a_emm_tm, &workspace, &tau_emm_tm);
        plant().SetPositions(context_, q_epp_t);
        plant().SetVelocities(context_, v_epp_t);
        CalcInverseDynamicsSingleTimeStep(*context_, a_epp_tm, &workspace, &tau_epp_tm);
        dtaum_dqt->col(i) =
            2.0 / 3.0 * (tau_ep_tm - tau_em_tm) / dq - 1.0 / 12.0 * (tau_epp_tm - tau_emm_tm) / dq;
      } else {
    ...
    if (t < num_steps()) {
      ...
      CalcInverseDynamicsSingleTimeStep(*context_, a_ep_t, &workspace, &tau_ep_t);

      plant().SetPositions(context_, q[t + 1]);
      plant().SetVelocities(context_, v_em_tp);
      CalcInverseDynamicsSingleTimeStep(*context_, a_em_t, &workspace, &tau_em_t);
      if (fourth_order) {
        plant().SetPositions(context_, q[t + 1]);
        plant().SetVelocities(context_, v_epp_tp);
        CalcInverseDynamicsSingleTimeStep(*context_, a_epp_t, &workspace, &tau_epp_t);

        plant().SetPositions(context_, q[t + 1]);
        plant().SetVelocities(context_, v_emm_tp);
        CalcInverseDynamicsSingleTimeStep(*context_, a_emm_t, &workspace, &tau_emm_t);
        dtaut_dqt->col(i) =
            2.0 / 3.0 * (tau_ep_t - tau_em_t) / dq - 1.0 / 12.0 * (tau_epp_t - tau_emm_t) / dq;
      } else {
    ...
    if (t < num_steps() - 1) {
      ...
      CalcInverseDynamicsSingleTimeStep(*context_, a_em_tp, &workspace, &tau_em_tp);
      plant().SetPositions(context_, q[t + 2]);
      plant().SetVelocities(context_, v[t + 2]);
      CalcInverseDynamicsSingleTimeStep(*context_, a_ep_tp, &workspace, &tau_ep_tp);
      if (fourth_order) {
        plant().SetPositions(context_, q[t + 2]);
        plant().SetVelocities(context_, v[t + 2]);
        CalcInverseDynamicsSingleTimeStep(*context_, a_emm_tp, &workspace, &tau_emm_tp);
        plant().SetPositions(context_, q[t + 2]);
        plant().SetVelocities(context_, v[t + 2]);
        CalcInverseDynamicsSingleTimeStep(*context_, a_epp_tp, &workspace, &tau_epp_tp);
        dtaup_dqt->col(i) =
            2.0 / 3.0 * (tau_ep_tp - tau_em_tp) / dq - 1.0 / 12.0 * (tau_epp_tp - tau_emm_tp) / dq;
      } else {
...
void TrajectoryOptimizer<AutoDiffXd>::CalcInverseDynamicsPartialsAutoDiff(
    const TrajectoryOptimizerState<double>&, InverseDynamicsPartials<double>*) const {}

template <typename T>
void TrajectoryOptimizer<T>::CalcInverseDynamicsPartialsAutoDiff(
    const TrajectoryOptimizerState<double>& state, InverseDynamicsPartials<double>* id_partials) const {
  DRAKE_DEMAND(id_partials->size() == num_steps());
  ...
  TrajectoryOptimizerWorkspace<AutoDiffXd> workspace_ad(num_steps(), *plant_ad_);
  ...
  for (int t = 1; t <= num_steps(); ++t) {
    ...
    const std::vector<VectorX<AutoDiffXd>>& a_ad = optimizer_ad_->EvalA(*state_ad_);

    // dtau_dqt[t].
    if (t < num_steps()) {
      const Context<AutoDiffXd>& context_ad_tp = optimizer_ad_->EvalPlantContext(*state_ad_, t + 1);
      optimizer_ad_->CalcInverseDynamicsSingleTimeStep(context_ad_tp, a_ad[t], &workspace_ad, &tau_ad);
      dtau_dqt[t] = drake::math::ExtractGradient(tau_ad);
    }

    // dtau_dqt[t+1].
    if (t < num_steps() - 1) {
      const Context<AutoDiffXd>& context_ad_tpp = optimizer_ad_->EvalPlantContext(*state_ad_, t + 2);
      optimizer_ad_->CalcInverseDynamicsSingleTimeStep(context_ad_tpp, a_ad[t + 1], &workspace_ad, &tau_ad);
      dtau_dqm[t + 1] = drake::math::ExtractGradient(tau_ad);
    }

    // dtau_dqt[t-1].
    if (t > 0) {
      const Context<AutoDiffXd>& context_ad_t = optimizer_ad_->EvalPlantContext(*state_ad_, t);
      optimizer_ad_->CalcInverseDynamicsSingleTimeStep(context_ad_t, a_ad[t - 1], &workspace_ad, &tau_ad);
...
template <typename T>
void TrajectoryOptimizer<T>::CalcVelocityPartials(const TrajectoryOptimizerState<T>& state,
...
template <typename T>
void TrajectoryOptimizer<T>::CalcGradientFiniteDiff(const TrajectoryOptimizerState<T>& state,
...
template <typename T>
void TrajectoryOptimizer<T>::CalcGradient(const TrajectoryOptimizerState<T>& state,
                                          drake::EigenPtr<VectorX<T>> g) const {
...
template <typename T>
const VectorX<T>& TrajectoryOptimizer<T>::EvalGradient(const TrajectoryOptimizerState<T>& state) const {
...
template <typename T>
void TrajectoryOptimizer<T>::CalcHessian(const TrajectoryOptimizerState<T>& state,
...
template <typename T>
void TrajectoryOptimizer<T>::CalcScaledHessian(const TrajectoryOptimizerState<T>& state,
...
template <typename T>
void TrajectoryOptimizer<T>::CalcScaledGradient(const TrajectoryOptimizerState<T>& state,
...
template <typename T>
const VectorX<T>& TrajectoryOptimizer<T>::EvalScaledGradient(const TrajectoryOptimizerState<T>& state) const {
  // Early exit if we're not using scaling
...
template <typename T>
void TrajectoryOptimizer<T>::CalcScaleFactors(const TrajectoryOptimizerState<T>& state, VectorX<T>* D) const {
  using std::min;
  using std::sqrt;
...
template <typename T>
const VectorX<T>& TrajectoryOptimizer<T>::EvalScaleFactors(const TrajectoryOptimizerState<T>& state) const {
...
template <typename T>
void TrajectoryOptimizer<T>::CalcEqualityConstraintViolations(const TrajectoryOptimizerState<T>& state,
...
template <typename T>
void TrajectoryOptimizer<T>::CalcEqualityConstraintJacobian(const TrajectoryOptimizerState<T>& state,
                                                            MatrixX<T>* J) const {
...
template <typename T>
void TrajectoryOptimizer<T>::CalcLagrangeMultipliers(const TrajectoryOptimizerState<T>&, VectorX<T>*) const {
...
template <>
void TrajectoryOptimizer<double>::CalcLagrangeMultipliers(const TrajectoryOptimizerState<double>& state,
...
template <typename T>
void TrajectoryOptimizer<T>::CalcMeritFunctionGradient(const TrajectoryOptimizerState<T>& state,
...
template <typename T>
void TrajectoryOptimizer<T>::CalcExactHessian(const TrajectoryOptimizerState<T>&,
                                              PentaDiagonalMatrix<T>*) const {
  throw std::runtime_error("TrajectoryOptimizer::CalcExactHessian only supports T=double");
}

template <>
void TrajectoryOptimizer<double>::CalcExactHessian(const TrajectoryOptimizerState<double>& state,
  ...
  const std::vector<VectorX<double>>& q = state.q();
  std::vector<VectorX<AutoDiffXd>> q_ad(num_steps() + 1, VectorX<AutoDiffXd>(nq));

  // Initialize q_ad
  int ad_idx = 0;
  for (int t = 0; t <= num_steps(); ++t) {
    for (int i = 0; i < nq; ++i) {
      q_ad[t].segment<1>(i) = drake::math::InitializeAutoDiff(q[t].segment<1>(i), num_vars, ad_idx);
      ++ad_idx;
    }
  }
  state_ad_->set_q(q_ad);
...
template <typename T>
const Context<T>& TrajectoryOptimizer<T>::EvalPlantContext(const TrajectoryOptimizerState<T>& state,
                                                           int t) const {
...
template <typename T>
Context<T>& TrajectoryOptimizer<T>::GetMutablePlantContext(const TrajectoryOptimizerState<T>& state,
                                                           int t) const {
...
template <typename T>
const std::vector<VectorX<T>>& TrajectoryOptimizer<T>::EvalV(const TrajectoryOptimizerState<T>& state) const {
  if (!state.cache().trajectory_data.up_to_date) CalcCacheTrajectoryData(state);
  return state.cache().trajectory_data.v;
}

template <typename T>
const std::vector<VectorX<T>>& TrajectoryOptimizer<T>::EvalA(const TrajectoryOptimizerState<T>& state) const {
  if (!state.cache().trajectory_data.up_to_date) CalcCacheTrajectoryData(state);
...
const VelocityPartials<T>& TrajectoryOptimizer<T>::EvalVelocityPartials(
    const TrajectoryOptimizerState<T>& state) const {
  if (!state.cache().derivatives_data.up_to_date) CalcCacheDerivativesData(state);
  return state.cache().derivatives_data.v_partials;
}

template <typename T>
const InverseDynamicsPartials<T>& TrajectoryOptimizer<T>::EvalInverseDynamicsPartials(
    const TrajectoryOptimizerState<T>& state) const {
  if (!state.cache().derivatives_data.up_to_date) CalcCacheDerivativesData(state);
...
template <typename T>
void TrajectoryOptimizer<T>::SaveLinePlotDataFirstVariable(TrajectoryOptimizerState<T>* scratch_state) const {
...
template <typename T>
void TrajectoryOptimizer<T>::SaveIterationData(const int iter, const double Delta, const double rho,
                                               const double dq,
                                               const TrajectoryOptimizerState<T>& state) const {
  std::ofstream data_file;
...
void TrajectoryOptimizer<T>::SaveContourPlotDataFirstTwoVariables(
  ...
  for (int i = 0; i < nq1; ++i) {
    ...
    for (int j = 0; j < nq2; ++j) {
      ...
      // Write to the file
      data_file << fmt::format("{}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}\n", q1, q2, cost, g(2), g(3),
                               H(2, 2), H(2, 3), H(3, 2), H(3, 3), g.norm(), H.block(2, 2, 2, 2).norm());
...
void TrajectoryOptimizer<T>::SaveQuadraticDataFirstTwoVariables(
  ...
  data_file << fmt::format("{}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}\n", iter, q1, q2, dq1,
                           dq2, Delta, EvalCost(state), g1, g2, H11, H12, H21, H22, g.norm(),
                           H.block(2, 2, 2, 2).norm());
  data_file.close();
}

template <typename T>
void TrajectoryOptimizer<T>::SaveLinesearchResidual(const TrajectoryOptimizerState<T>& state,
                                                    const VectorX<T>& dq,
...
template <typename T>
T TrajectoryOptimizer<T>::CalcTrustRatio(const TrajectoryOptimizerState<T>& state, const VectorX<T>& dq,
                                         TrajectoryOptimizerState<T>* scratch_state) const {
...
template <typename T>
void TrajectoryOptimizer<T>::SolveLinearSystemInPlace(const PentaDiagonalMatrix<T>&,
                                                      drake::EigenPtr<VectorX<T>>) const {
...
template <>
void TrajectoryOptimizer<double>::SolveLinearSystemInPlace(const PentaDiagonalMatrix<double>& H,
...
template <typename T>
bool TrajectoryOptimizer<T>::CalcDoglegPoint(const TrajectoryOptimizerState<T>&, const double, VectorX<T>*,
                                             VectorX<T>*) const {
...
template <typename T>
SolverFlag TrajectoryOptimizer<T>::Solve(const std::vector<VectorX<T>>&, TrajectoryOptimizerSolution<T>*,
...
template <>
SolverFlag TrajectoryOptimizer<double>::Solve(const std::vector<VectorXd>& q_guess,
...
template <>
SolverFlag TrajectoryOptimizer<double>::SolveWithLinesearch(const std::vector<VectorXd>& q_guess,
                                                            TrajectoryOptimizerSolution<double>* solution,
  ...
  do {
    ...
    if (params_.linesearch_plot_every_iteration) {
      SaveLinesearchResidual(state, dq, &scratch_state, fmt::format("linesearch_data_{}.csv", k));
    }
    ...
    if (ls_iters >= params_.max_linesearch_iterations) {
      ...
      if (params_.verbose) {
        ...
        std::cout << "Reached maximum linesearch iterations (" << params_.max_linesearch_iterations << ")."
                  << std::endl;
      }
...
template <>
SolverFlag TrajectoryOptimizer<double>::SolveWithTrustRegion(const std::vector<VectorXd>& q_guess,
                                                             TrajectoryOptimizerSolution<double>* solution,
                                                             TrajectoryOptimizerStats<double>* stats,
  ...
  WarmStart warm_start(num_steps(), diagram(), plant(), num_equality_constraints(), q_guess, params_.Delta0);
...
template <typename T>
SolverFlag TrajectoryOptimizer<T>::SolveFromWarmStart(WarmStart*, TrajectoryOptimizerSolution<T>*,
                                                      TrajectoryOptimizerStats<T>*,
                                                      ConvergenceReason*) const {
  ...
  while (k < params_.max_iterations) {
    ...
    if (params_.verbose) {
      ...
      std::cout << fmt::format(
          ...
          "{:>10.4} | {:>10.4} |\n",
          k, cost, Delta, rho, iter_time.count(), g.norm() / cost, dL_dq, h.norm());
    }
...
template <typename T>
void TrajectoryOptimizer<T>::NormalizeQuaternions(TrajectoryOptimizerState<T>* state) const {
