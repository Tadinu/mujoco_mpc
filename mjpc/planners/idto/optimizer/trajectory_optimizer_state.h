...
struct TrajectoryOptimizerCache {
  ...
  TrajectoryOptimizerCache(const int num_steps, const Diagram<T>& diagram, const MultibodyPlant<T>& plant,
                           const int num_eq_constraints)
      : TrajectoryOptimizerCache(num_steps, plant.num_velocities(), plant.num_positions(),
                                 num_eq_constraints) {
  ...
  struct ContextCache {
    ...
    // diagram and plant.
    ContextCache(int num_steps, const Diagram<T>& diagram, const MultibodyPlant<T>& plant) {
      diagram_contexts.reserve(num_steps + 1);
      plant_contexts.reserve(num_steps + 1);
      for (int t = 0; t <= num_steps; ++t) {
        auto context_t = diagram.CreateDefaultContext();
        plant_contexts.push_back(&diagram.GetMutableSubsystemContext(plant, context_t.get()));
...
public:
   ...
   * @param num_eq_constraints number of equality constraints
   */
  TrajectoryOptimizerState(const int num_steps, const Diagram<T>& diagram, const MultibodyPlant<T>& plant,
                           const int num_eq_constraints)
    ...
    per_timestep_workspace.assign(num_steps + 1, TrajectoryOptimizerWorkspace<T>(num_steps, plant));
  }
