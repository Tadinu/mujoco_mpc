...
#include "mjpc/planners/idto/optimizer/inverse_dynamics_partials.h"
#include "mjpc/planners/idto/optimizer/penta_diagonal_matrix.h"
#include "mjpc/planners/idto/optimizer/problem_definition.h"
#include "mjpc/planners/idto/optimizer/solver_parameters.h"
#include "mjpc/planners/idto/optimizer/trajectory_optimizer_solution.h"
#include "mjpc/planners/idto/optimizer/trajectory_optimizer_state.h"
#include "mjpc/planners/idto/optimizer/trajectory_optimizer_workspace.h"
#include "mjpc/planners/idto/optimizer/velocity_partials.h"
#include "mjpc/planners/idto/optimizer/warm_start.h"
...
public:
  ...
  TrajectoryOptimizer(const Diagram<T>* diagram, const MultibodyPlant<T>* plant,
                      const ProblemDefinition& prob, const SolverParameters& params = SolverParameters{});
  ...
  TrajectoryOptimizerState<T> CreateState() const {
    ...
    return TrajectoryOptimizerState<T>(num_steps(), diagram(), plant(), num_equality_constraints());
  }
   ...
   */
  void CalcGradient(const TrajectoryOptimizerState<T>& state, drake::EigenPtr<VectorX<T>> g) const;
   ...
   * @return SolverFlag
   */
  SolverFlag Solve(const std::vector<VectorX<T>>& q_guess, TrajectoryOptimizerSolution<T>* solution,
                   TrajectoryOptimizerStats<T>* stats, ConvergenceReason* reason = nullptr) const;
   ...
   * @return SolverFlag
   */
  SolverFlag SolveFromWarmStart(WarmStart* warm_start, TrajectoryOptimizerSolution<T>* solution,
   ...
   */
  const std::vector<VectorX<T>>& EvalV(const TrajectoryOptimizerState<T>& state) const;
   ...
   */
  const std::vector<VectorX<T>>& EvalA(const TrajectoryOptimizerState<T>& state) const;
   ...
   */
  const std::vector<VectorX<T>>& EvalTau(const TrajectoryOptimizerState<T>& state) const;
   ...
   */
  const std::vector<MatrixX<T>>& EvalNplus(const TrajectoryOptimizerState<T>& state) const;
   ...
   */
  const VectorX<T>& EvalScaledGradient(const TrajectoryOptimizerState<T>& state) const;
   ...
   */
  const VectorX<T>& EvalScaleFactors(const TrajectoryOptimizerState<T>& state) const;
   ...
   */
  const VectorX<T>& EvalEqualityConstraintViolations(const TrajectoryOptimizerState<T>& state) const;
   ...
   */
  const MatrixX<T>& EvalEqualityConstraintJacobian(const TrajectoryOptimizerState<T>& state) const;
   ...
   */
  const VectorX<T>& EvalLagrangeMultipliers(const TrajectoryOptimizerState<T>& state) const;
   ...
   */
  const VectorX<T>& EvalMeritFunctionGradient(const TrajectoryOptimizerState<T>& state) const;
   ...
   */
  const VectorX<T>& EvalGradient(const TrajectoryOptimizerState<T>& state) const;
  ...
  void ResetInitialConditions(const VectorXd& q_init, const VectorXd& v_init) {
    ...
    DRAKE_DEMAND(params_.q_nom_relative_to_q_init.size() == plant().num_positions());
    prob_.q_init = q_init;
    prob_.v_init = v_init;
  }
   ...
   */
  void UpdateNominalTrajectory(const std::vector<VectorXd>& q_nom, const std::vector<VectorXd>& v_nom) {
...
private:
   ...
   */
  void CalcContextCache(const TrajectoryOptimizerState<T>& state,
  ...
  void CalcInverseDynamicsCache(const TrajectoryOptimizerState<T>& state,
  ...
  T CalcCost(const std::vector<VectorX<T>>& q, const std::vector<VectorX<T>>& v,
             const std::vector<VectorX<T>>& tau, TrajectoryOptimizerWorkspace<T>* workspace) const;
   ...
   */
  void CalcVelocities(const std::vector<VectorX<T>>& q, const std::vector<MatrixX<T>>& Nplus,
   ...
   */
  void CalcAccelerations(const std::vector<VectorX<T>>& v, std::vector<VectorX<T>>* a) const;
   ...
   */
  void CalcInverseDynamics(const TrajectoryOptimizerState<T>& state, const std::vector<VectorX<T>>& a,
   ...
   */
  void CalcInverseDynamicsSingleTimeStep(const Context<T>& context, const VectorX<T>& a,
   ...
   */
  void CalcContactForceContribution(const Context<T>& context, MultibodyForces<T>* forces) const;
   ...
   */
  void CalcNplus(const TrajectoryOptimizerState<T>& state, std::vector<MatrixX<T>>* N_plus) const;
   ...
   */
  void CalcInverseDynamicsPartials(const TrajectoryOptimizerState<T>& state,
   ...
   * @param id_partials struct for holding dtau/dq
   */
  void CalcInverseDynamicsPartialsFiniteDiff(const TrajectoryOptimizerState<T>& state,
                                             InverseDynamicsPartials<T>* id_partials) const;
   ...
   * @param id_partials struct for holding dtau/dq
   */
  void CalcInverseDynamicsPartialsCentralDiff(const TrajectoryOptimizerState<T>& state,
                                              InverseDynamicsPartials<T>* id_partials) const;
   ...
   */
  void CalcInverseDynamicsPartialsWrtQtCentralDiff(int t, const TrajectoryOptimizerState<T>& state,
                                                   MatrixX<T>* dtaum_dqt, MatrixX<T>* dtaut_dqt,
   ...
   * @param id_partials struct for holding dtau/dq
   */
  void CalcInverseDynamicsPartialsAutoDiff(const TrajectoryOptimizerState<double>& state,
   ...
   *          decision variable (q_t[i]).
   */
  void CalcGradientFiniteDiff(const TrajectoryOptimizerState<T>& state, drake::EigenPtr<VectorX<T>> g) const;
   ...
   */
  std::tuple<double, int> Linesearch(const TrajectoryOptimizerState<T>& state, const VectorX<T>& dq,
   ...
   */
  void SaveLinesearchResidual(const TrajectoryOptimizerState<T>& state, const VectorX<T>& dq,
   ...
   *    L(q + alpha*dq) < L(q) + c*g'*dq
   */
  std::tuple<double, int> ArmijoLinesearch(const TrajectoryOptimizerState<T>& state, const VectorX<T>& dq,
                                           TrajectoryOptimizerState<T>* scratch_state) const;
   ...
   */
  T CalcTrustRatio(const TrajectoryOptimizerState<T>& state, const VectorX<T>& dq,
                   TrajectoryOptimizerState<T>* scratch_state) const;
   ...
   */
  bool CalcDoglegPoint(const TrajectoryOptimizerState<T>& state, const double Delta, VectorX<T>* dq,
                       VectorX<T>* dqH) const;
   ...
   */
  void SolveLinearSystemInPlace(const PentaDiagonalMatrix<T>& H, drake::EigenPtr<VectorX<T>> b) const;

  ConvergenceReason VerifyConvergenceCriteria(const TrajectoryOptimizerState<T>& state,
                                              const T& previous_cost, const VectorX<T>& dq) const;
   ...
   * values of q.
   */
  void SaveContourPlotDataFirstTwoVariables(TrajectoryOptimizerState<T>* scratch_state) const;
   ...
   */
  void SaveLinePlotDataFirstVariable(TrajectoryOptimizerState<T>* scratch_state) const;
   ...
   * etc
   */
  void SaveIterationData(const int iter_num, const double Delta, const double dq, const double rho,
                         const TrajectoryOptimizerState<T>& state) const;
   ...
   * and H
   */
  void SaveQuadraticDataFirstTwoVariables(const int iter_num, const double Delta, const VectorX<T>& dq,
                                          const TrajectoryOptimizerState<T>& state) const;
   ...
   */
  void CalcScaledGradient(const TrajectoryOptimizerState<T>& state, VectorX<T>* gtilde) const;
   ...
   */
  void CalcScaleFactors(const TrajectoryOptimizerState<T>& state, VectorX<T>* D) const;
   ...
   */
  void CalcEqualityConstraintViolations(const TrajectoryOptimizerState<T>& state,
   ...
   */
  void CalcEqualityConstraintJacobian(const TrajectoryOptimizerState<T>& state, MatrixX<T>* J) const;
   ...
   */
  void CalcLagrangeMultipliers(const TrajectoryOptimizerState<T>& state, VectorX<T>* lambda) const;
   ...
   */
  void CalcMeritFunctionGradient(const TrajectoryOptimizerState<T>& state, VectorX<T>* g_tilde) const;
...
template <>
SolverFlag TrajectoryOptimizer<double>::SolveWithLinesearch(const std::vector<VectorXd>&,
                                                            TrajectoryOptimizerSolution<double>*,
                                                            TrajectoryOptimizerStats<double>*) const;

template <>
SolverFlag TrajectoryOptimizer<double>::SolveWithTrustRegion(const std::vector<VectorXd>&,
                                                             TrajectoryOptimizerSolution<double>*,
                                                             TrajectoryOptimizerStats<double>*,
                                                             ConvergenceReason*) const;

template <>
SolverFlag TrajectoryOptimizer<double>::SolveFromWarmStart(WarmStart*, TrajectoryOptimizerSolution<double>*,
                                                           TrajectoryOptimizerStats<double>*,
                                                           ConvergenceReason*) const;
