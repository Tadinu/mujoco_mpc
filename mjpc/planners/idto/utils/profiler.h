...
using TimerIndex = drake::TypeSafeIndex<class TimerTag>;
...
public:
  ...
  double average() const {
    const double total_time =
        std::chrono::duration_cast<std::chrono::duration<double, Units>>(total_).count();
  ...
  template <typename Units>
  double total() const {
    return std::chrono::duration_cast<std::chrono::duration<double, Units>>(total_).count();
  }
...
#define INSTRUMENT_FUNCTION(details_string)                                        \
  ...
  static const drake::common::TimerIndex timer = addTimer(std::move(description)); \
  PushTimer(timer);                                                                \
  startTimer(timer);                                                               \
