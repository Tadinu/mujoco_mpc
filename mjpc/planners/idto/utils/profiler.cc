...
void startTimer(TimerIndex index) { Profiler::getMutableInstance().start(index); }

void stopTimer(TimerIndex index) { Profiler::getMutableInstance().lap<LapTimer::Units>(index); }

TimerIndex PushTimer(TimerIndex index) { return Profiler::getMutableInstance().PushTimer(index); }
...
double lapTimer(TimerIndex index) { return Profiler::getMutableInstance().lap<LapTimer::Units>(index); }
...
std::string TableOfAverages() {
  ...
  ss << fmt::format("{:>15}{:>10}{:>17}{:>11} {:>10}", "Time/sample (s)", "Samples", "Total Time (s)",
                    "Self (s)", "Label")
     << "\n";
  double self_total = 0;
  for (TimerIndex i(0); i < count; ++i) {
    double time = profiler.average<Seconds>(i);
    ss << fmt::format("{:>15.7g}{:>10}{:>17.7g}{:>15.7g}  {}", time, profiler.laps(i),
                      profiler.total<Seconds>(i), profiler.self_time(i), profiler.displayString(i));
    // if (i < count - 1)
    ss << "\n";
