...
void IdtoPlanner::Initialize(mjModel* model, const mjpc::Task& task) {
  task_ = const_cast<mjpc::Task*>(&task);
  model_ = model;
  data_ = task_->data_;
  meshcat_ = mjpc::Task::Meshcat();
  configs_ = task_->idto_configs_;
...
void IdtoPlanner::StartControl() {
  ...
  if (configs_->mpc) {
    ...
    SolveTrajectoryOptimization(*configs_);
  }
}

void IdtoPlanner::RunMPC(const IdtoPlannerConfig& options) const {
  ...
  TrajectoryOptimizerSolution<double> initial_solution = SolveTrajectoryOptimization(options);
  ...
  task_->CreateDrakePlantModel(&plant);
  plant.Finalize();
  ...
  auto [ctrl_plant, ctrl_scene_graph] = AddMultibodyPlant(ctrl_config, &ctrl_builder);
  task_->CreateDrakePlantModel(&ctrl_plant);
  ctrl_plant.Finalize();
  ...
  auto controller = builder.AddSystem<IdtoMPC>(ctrl_diagram.get(), &ctrl_plant, opt_prob, initial_solution,
                                               solver_params, replan_period);

  // Create an interpolator to send samples from the optimal trajectory at a
  // faster rate
  auto interpolator = builder.AddSystem<IdtoInterpolator>(nq, nv, nu);
  ...
  IdtoStoredTrajectory placeholder_trajectory;
  controller->StoreOptimizerSolution(initial_solution, 0.0, &placeholder_trajectory);

  auto delay = builder.AddSystem<DiscreteTimeDelay>(replan_period, 1, drake::Value(placeholder_trajectory));
  builder.Connect(controller->get_trajectory_output_port(), delay->get_input_port());
  builder.Connect(delay->get_output_port(), interpolator->get_trajectory_input_port());
  ...
  const MatrixXd Kp = (options.Kp.size() == 0)
                          ? MatrixXd::Zero(nu, nq)
                          : static_cast<MatrixXd>(Bq.transpose() * options.Kp.asDiagonal());
  const MatrixXd Kd = (options.Kd.size() == 0)
                          ? MatrixXd::Zero(nu, nv)
                          : static_cast<MatrixXd>(B.transpose() * options.Kd.asDiagonal());

  auto pd = builder.AddSystem<IdtoPdPlusController>(Kp, Kd, options.feed_forward);
  builder.Connect(interpolator->get_state_output_port(), pd->get_nominal_state_input_port());
  builder.Connect(interpolator->get_control_output_port(), pd->get_nominal_control_input_port());
  builder.Connect(plant.get_state_output_port(), pd->get_state_input_port());
  builder.Connect(pd->get_control_output_port(), plant.get_actuation_input_port());
  ...
  std::cout << ("Profiling turned off.\n"
                "Enable profiling by running with --copt=-DENABLE_TIMERS.")
            << std::endl;
}

TrajectoryOptimizerSolution<double> IdtoPlanner::SolveTrajectoryOptimization(
    const IdtoPlannerConfig& options) const {
  ...
  std::vector<VectorXd> q_guess =
      MakeLinearInterpolation(opt_prob.q_init, options.q_guess, opt_prob.num_steps + 1);
  ...
  if (status == SolverFlag::kSuccess) {
    ...
    std::cout << "Maximum iterations reached in " << stats.solve_time << " seconds." << std::endl;
  } else {
    std::cout << "Solver failed!" << std::endl;
  }

  std::cout << "Convergence reason: " << DecodeConvergenceReasons(reason) + ".\n";
  ...
  std::cout << std::endl;
  std::cout << fmt::format("Max torques: {}", drake::fmt_eigen(tau_max.transpose())) << std::endl;
  ...
  std::cout << std::endl;
  std::cout << fmt::format("q_nom[T] : {}", drake::fmt_eigen(opt_prob.q_nom[options.num_steps].transpose()))
            << std::endl;
  std::cout << fmt::format("q[T]     : {}", drake::fmt_eigen(solution.q[options.num_steps].transpose()))
            << std::endl;
  std::cout << std::endl;
  std::cout << fmt::format("v_nom[T] : {}", drake::fmt_eigen(opt_prob.v_nom[options.num_steps].transpose()))
            << std::endl;
  std::cout << fmt::format("v[T]     : {}", drake::fmt_eigen(solution.v[options.num_steps].transpose()))
            << std::endl;
...
void IdtoPlanner::PlayBackTrajectory(const std::vector<VectorXd>& q, const double time_step) const {
...
void IdtoPlanner::SetProblemDefinition(const IdtoPlannerConfig& options, const MultibodyPlant<double>& plant,
  ...
  opt_prob->q_nom = MakeLinearInterpolation(q_nom_start, q_nom_end, options.num_steps + 1);
  ...
  for (int t = 1; t <= options.num_steps; ++t) {
    if (options.q_init.size() == options.v_init.size()) {
      ...
      opt_prob->v_nom.push_back((opt_prob->q_nom[t] - opt_prob->q_nom[t - 1]) / options.time_step);
    } else {
...
void IdtoPlanner::SetSolverParameters(const IdtoPlannerConfig& options,
  ...
  if (options.linesearch == "backtracking") {
    ...
    throw std::runtime_error(fmt::format("Unknown linesearch method '{}'", options.linesearch));
  }
  ...
  if (options.gradients_method == "forward_differences") {
    ...
    throw std::runtime_error(fmt::format("Unknown gradient method '{}'", options.gradients_method));
  }
  ...
  if (options.method == "linesearch") {
    ...
    throw std::runtime_error(fmt::format("Unknown solver method '{}'", options.method));
  }

  if (options.linear_solver == "pentadiagonal_lu") {
    solver_params->linear_solver = SolverParameters::LinearSolverType::kPentaDiagonalLu;
  } else if (options.linear_solver == "dense_ldlt") {
    solver_params->linear_solver = SolverParameters::LinearSolverType::kDenseLdlt;
  } else {
    throw std::runtime_error(fmt::format("Unknown linear solver '{}'", options.linear_solver));
  }
  ...
  solver_params->linesearch_plot_every_iteration = options.linesearch_plot_every_iteration;
  ...
  if (options.scaling_method == "sqrt") {
    ...
    throw std::runtime_error(fmt::format("Unknown scaling method '{}'", options.scaling_method));
  }
...
void IdtoPlanner::NormalizeQuaternions(const MultibodyPlant<double>& plant, std::vector<VectorXd>* q) const {
...
void IdtoPlanner::NormalizeQuaternions(const MultibodyPlant<double>& plant, VectorXd* q) const {
