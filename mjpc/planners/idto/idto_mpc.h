...
class IdtoMPC : public LeafSystem<double> {
public:
  /**
   ...
   */
  IdtoMPC(const Diagram<double>* diagram, const MultibodyPlant<double>* plant, const ProblemDefinition& prob,
          const TrajectoryOptimizerSolution<double>& warm_start_solution, const SolverParameters& params,
          const double replan_period);

  const InputPort<double>& get_state_input_port() const { return this->get_input_port(state_input_port_); }
   ...
   */
  void StoreOptimizerSolution(const TrajectoryOptimizerSolution<double>& solution, const double start_time,
                              IdtoStoredTrajectory* stored_trajectory) const;
...
private:
   ...
   */
  EventStatus UpdateAbstractState(const Context<double>& context, State<double>* state) const;
   ...
   */
  void UpdateInitialGuess(const IdtoStoredTrajectory& stored_trajectory, const double current_time,
...
/// frequency.
class IdtoInterpolator : public LeafSystem<double> {
public:
  IdtoInterpolator(const int nq, const int nv, const int nu);
...
private:
   ...
   */
  void SendState(const Context<double>& context, BasicVector<double>* output) const;
   ...
   * @param output output that we'll set
   */
  void SendControl(const Context<double>& context, BasicVector<double>* output) const;
