...
public:
  ...
  void InitTaskCIO() {
    optimizer_ =
        std::make_shared<RiemannianOptimizer>(model, task->data_, const_cast<Task*>(task), this, action_dim_);
  }
  ...
  CostDerivatives ComputeDerivatives(const TrajectoryPtr& in_trajectory) {
    ...
    model_derivative.Compute(model, data_, in_trajectory->states.data(), in_trajectory->actions.data(),
                             in_trajectory->times.data(), dim_state, dim_state_derivative, action_dim_,
