...
namespace cio_utils {
template <typename T>
static void print_array(T* val, int size) {
  std::vector<T> vec(size);
  std::memcpy(vec.data(), val, size * sizeof(T));
  std::cout << vec << std::endl;
}
template <typename T>
static void print_vec3(T* val) {
  print_array(val, 3);
}
...
static std::vector<Eigen::Vector3d> linspace_vectors(const Eigen::Vector3d& vec0, const Eigen::Vector3d& vec1,
                                                     int num_steps) {
