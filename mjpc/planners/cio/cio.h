...
using CIOLinearVelocity = Eigen::Vector3d;
struct CIOVelocity {
...
struct CIOTrajectory {
  std::vector < std::vector<CIOContactState> contact_states;
};

void L(S, goals, world, p, stage = 0) {
    def L_CI(t, world_t):
        cost = 0
...
void CIO(goals, world, p, single = False, start_stage = 0, traj_data = None, gif_tag ='') {
  if (single) {
    // FOR TESTING A SINGLE traj
    S = world.traj_func(world, goals, p, traj_data) S =
        add_noise(S) visualize_result(world, goals, p, 'initial' + gif_tag + '.gif', S) tot_cost =
            L(S, goals, world, p, start_stage) print_final(*function_costs) return {};
  }
