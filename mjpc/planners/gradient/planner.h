...
class GradientPlanner : public Planner {
 public:
  ...
  void OptimizePolicy(int horizon, ThreadPool& pool) override {
    ...
    for (int i = 0; i < settings.max_rollout; i++) {
      ...
      for (int j = 1; j < num_trajectory; j++) {
        candidate_policy[j].CopyFrom(candidate_policy[0],
  ...
  void ActionFromPolicy(double* action, const double* state_,
                        double time_, bool use_previous) override {
    const std::shared_lock<std::shared_mutex> lock(mtx_);
    if (use_previous) {
      previous_policy.Action(action, state_, time_);
    } else {
      policy.Action(action, state_, time_);
    }
  }
