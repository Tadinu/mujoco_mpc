...
class iLQGPlanner : public Planner {
 public:
  ...
  void ActionFromPolicy(double* action_, const double* state_,
                        double time_, bool use_previous_)override {
    const std::shared_lock<std::shared_mutex> lock(mtx_);
    if (use_previous_) {
    ...
    for (int i = 0; i < num_trajectory_; i++) {
      pool.Schedule([&data = data_, &trajectory = trajectory,
        ...
        auto feedback_policy = [&candidate_policy = candidate_policy, model, i](
                                  ...
                                  int index) {
          // dimensions
          int dim_state_ = model->nq + model->nv + model->na;
          int dim_state_derivative_ = 2 * model->nv + model->na;
          int dim_action_ = model->nu;

          // set improved action
          mju_copy(
              action,
              DataAt(candidate_policy[i].trajectory.actions, index * dim_action_),
              dim_action_);
          ...
          StateDiff(
              ...
              DataAt(candidate_policy[i].trajectory.states, index * dim_state_),
              state, 1.0);

          // compute feedback term
          mju_mulMatVec(candidate_policy[i].action_scratch.data(),
                        DataAt(candidate_policy[i].feedback_gain,
                              index * dim_action_ * dim_state_derivative_),
                        candidate_policy[i].state_scratch.data(), dim_action_,
          ...
          // clamp controls
          Clamp(action, model->actuator_ctrlrange, dim_action_);
        };
