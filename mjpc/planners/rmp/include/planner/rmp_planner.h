...
public:
  ...
  std::vector<std::shared_ptr<RMPPolicyBase<TSpace>>> getPolicies() override {
    ...
    (*simple_target_policy)(goal_pos, Matrix::Identity(), 0.8, 1.6, 1.0);

    static auto simple_target_policy1 = std::make_shared<SimpleTargetPolicy<TSpace>>();
    (*simple_target_policy1)(goal_pos, Matrix::Identity(), 1.6, 3.2, 1.0);
    static auto simple_target_policy2 = std::make_shared<SimpleTargetPolicy<TSpace>>();
    (*simple_target_policy2)(goal_pos, VectorQ::UnitX().asDiagonal(), 20.0, 44.0, 20);
    static auto simple_target_policy3 = std::make_shared<SimpleTargetPolicy<TSpace>>();
    (*simple_target_policy3)(goal_pos, VectorQ::UnitY().asDiagonal(), 20.0, 44.0, 20);
    ...
    static auto rmp_collision_policy =
        RMPPolicyBase<TSpace>::template MakePolicy<RaycastingPolicy<TSpace>, ERMPPolicyType::RAYCASTING>();
  ...
  void Initialize(mjModel* model, const mjpc::Task& task) override {
    ...
    // dimensions
    this->action_dim_ = model->nu; // action dimension
    ...
    dim_max = mju_max(mju_max(mju_max(dim_state, dim_state_derivative), this->action_dim_),
                      model->nuser_sensor);
  ...
  void Allocate() override {
    trajectory_->Initialize(dim_state, this->action_dim_, task_->num_residual, task_->num_trace, 1);
  ...
  void OptimizePolicy(int horizon, mjpc::ThreadPool& pool) override {
    const RMPSharedMutexLock lock(policy_mutex_);
  ...
  void ActionFromPolicy(double* action, const double* state, double time,
    ...
    // NOTE: Depending on which dofs the action ctrl is configured in xml, eg: linear movement only uses
    // linear_inertia only
  ...
  void DrawTrajectoryCurrent(const mjpc::Trajectory* trajectory, mjvScene* scn = nullptr,
                             const float* color = nullptr);
  void DrawTrajectory(const mjpc::Trajectory* trajectory, mjvScene* scn = nullptr,
                      const float* color = nullptr);
