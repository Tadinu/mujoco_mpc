...
class SampleGradientPlanner : public Planner {
 public:
  ...
  void Initialize(mjModel* model_, const Task& task_) override {
  ...
  // set state
  void SetState(const State& state_) override {
    state_.CopyTo(this->state.data(), this->mocap.data(), this->userdata.data(),
                &this->time);
  }
  ...
  void NominalTrajectory(int horizon, ThreadPool& pool) override {
    ...
    auto nominal_policy = [&cp = resampled_policy](
                           double* action_, const double* state_, double time_) {
      cp.Action(action_, state_, time_);
    };
  ...
  void ActionFromPolicy(double* action_,
                        const double* state_, double time_,
                        bool use_previous_) override {
    const std::shared_lock<std::shared_mutex> lock(mtx_);
    if (use_previous_) {
      previous_policy.Action(action_, state_, time_);
    } else {
      policy.Action(action_, state_, time_);
    }
  }

  // update policy via resampling
  void ResamplePolicy(
      SamplingPolicy& policy_, int horizon, int num_spline_points) {
    // dimension
    ...
    plan_scratch.SetInterpolation(policy_.plan.Interpolation());
    for (int t = 0; t < num_spline_points; t++) {
      TimeSpline::Node node = plan_scratch.AddNode(nominal_time);
      policy_.Action(node.values().data(), /*state=*/nullptr, nominal_time);
      nominal_time += time_shift;
    }

    // copy resampled policy parameters
    policy_.SetPlan(plan_scratch);

    // set dimensions
    policy_.num_spline_points = num_spline_points;
  }
  ...
  void Rollouts(int num_trajectory, int num_gradient,
  ...
  void GradientCandidates(int num_trajectory,
