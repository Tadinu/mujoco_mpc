...
void CrossEntropyPlanner::Reset(int horizon, const double* initial_repeated_action) {
  // state
...
void CrossEntropyPlanner::NominalTrajectory(int horizon) {
  ...
  nominal_trajectory->Rollout(nominal_policy, task, model, data_[ThreadPool::WorkerId()].get(), state.data(),
...
void CrossEntropyPlanner::ActionFromPolicy(double* action, const double* state, double time,
                                           bool use_previous) {
  const std::shared_lock<std::shared_mutex> lock(mtx_);
  if (use_previous) {
...
void CrossEntropyPlanner::Rollouts(int num_trajectory, int horizon, ThreadPool& pool) {
  ...
  for (int i = 0; i < num_trajectory; i++) {
    ...
    pool.Schedule([&s = *this, &model = this->model, &task = this->task, &state = this->state,
                   &time = this->time, &mocap = this->mocap, &userdata = this->userdata, horizon, std_min,
                   i]() {
      // copy nominal policy and sample noise
      {
        ...
        s.candidate_policy[i].plan.SetInterpolation(s.resampled_policy.plan.Interpolation());
...
const Trajectory* CrossEntropyPlanner::BestTrajectory() { return nominal_trajectory.get(); }
...
void CrossEntropyPlanner::Plots(mjvFigure* fig_planner, mjvFigure* fig_timer, int planner_shift,
