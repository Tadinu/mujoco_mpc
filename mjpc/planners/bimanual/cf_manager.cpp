...
void CfManager::init(
  ...
  // Reset [prediction_threads_]
  joinPredictionThreads();
  prediction_threads_.clear();
  int dim_size = 60;
...
std::vector<std::vector<Eigen::Vector3d>> CfManager::getPredictedPaths() const {
  std::vector<std::vector<Eigen::Vector3d>> predicted_paths;
  for (const auto& ee_agent : ee_agents_) {
    predicted_paths.push_back(ee_agent->getPath());
  }
  return predicted_paths;
}

std::vector<double> CfManager::getPredictedPathLengths() const {
  std::vector<double> path_lengths;
  for (const auto& ee_agent : ee_agents_) {
    path_lengths.push_back(ee_agent->getPathLength());
  }
  return path_lengths;
}

std::vector<double> CfManager::getPredictionTimes() const {
  std::vector<double> pred_time;
  for (const auto& ee_agent : ee_agents_) {
    pred_time.push_back(ee_agent->getPredictionTime());
  }
  return pred_time;
}

std::vector<bool> CfManager::getAgentSuccess() const {
  std::vector<bool> success;
  for (const auto& ee_agent : ee_agents_) {
    success.push_back(ee_agent->getReachedGoal());
  }
  return success;
}
