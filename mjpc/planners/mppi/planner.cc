...
// Modified from https://github.com/google-deepmind/mujoco_mpc/pull/257
...
void MPPIPlanner::OptimizePolicy(int horizon, ThreadPool& pool) {
  ...
  for (int i = 0; i < num_trajectory; i++) {
    ...
    mju_addToScl(parameters_scratch.data(), candidate_policy[i].parameters.data(), weight_vec[i] / denom,
                 policy.num_parameters);

    // add to nominal trajectory
    mju_addToScl(nominal_trajectory.actions.data(), trajectory[i].actions.data(), weight_vec[i] / denom,
                 model->nu * (horizon - 1));
    mju_addToScl(nominal_trajectory.trace.data(), trajectory[i].trace.data(), weight_vec[i] / denom,
                 3 * horizon);
    mju_addToScl(nominal_trajectory.residual.data(), trajectory[i].residual.data(), weight_vec[i] / denom,
                 nominal_trajectory.dim_residual * horizon);
    mju_addToScl(nominal_trajectory.costs.data(), trajectory[i].costs.data(), weight_vec[i] / denom, horizon);
    nominal_trajectory.total_return += trajectory[i].total_return * weight_vec[i] / denom;
  }
  ...
  improvement = mju_max(nominal_trajectory.total_return - trajectory[idx].total_return, 0.0);
...
void MPPIPlanner::NominalTrajectory(int horizon, ThreadPool& pool) {
  ...
  nominal_trajectory.Rollout(nominal_policy, task, model, data_[0].get(), state.data(), time, mocap.data(),
                             userdata.data(), horizon);
}

// set action from policy
void MPPIPlanner::ActionFromPolicy(double* action, const double* state, double time, bool use_previous) {
  const std::shared_lock<std::shared_mutex> lock(mtx_);
  if (use_previous) {
...
void MPPIPlanner::ResamplePolicy(int horizon) {
  ...
  for (int t = 0; t < num_spline_points; t++) {
    ...
    resampled_policy.Action(DataAt(parameters_scratch, t * model->nu), nullptr, nominal_time);
    nominal_time += time_shift;
  }

  // copy resampled policy parameters
  mju_copy(resampled_policy.parameters.data(), parameters_scratch.data(), num_parameters);
  mju_copy(resampled_policy.times.data(), times_scratch.data(), num_spline_points);

#if 0
  // time step power scaling
  PowerSequence(resampled_policy.times.data(), time_shift, resampled_policy.times[0],
                resampled_policy.times[num_spline_points - 1], timestep_power, num_spline_points);
#endif
}
...
void MPPIPlanner::AddNoiseToPolicy(int i) {
  ...
  // add noise
  mju_addTo(candidate_policy[i].parameters.data(), DataAt(noise, shift), num_parameters);

  // clamp parameters
  for (int t = 0; t < num_spline_points; t++) {
    Clamp(DataAt(candidate_policy[i].parameters, t * model->nu), model->actuator_ctrlrange, model->nu);
  }
  ...
  for (int i = 0; i < num_trajectory; i++) {
    pool.Schedule([&s = *this, &model = this->model, &task = this->task, &state = this->state,
      ...
      // copy nominal policy and sample noise
      {
        ...
        s.candidate_policy[i].representation = s.resampled_policy.representation;
  ...
  for (int k = 0; k < num_trajectory_; k++) {
    ...
    for (int i = 0; i < best->horizon - 1; i++) {
      ...
      for (int j = 0; j < task->num_trace; j++) {
        ...
        // make geometry
        mjv_makeConnector(&scn->geoms[scn->ngeom], mjGEOM_LINE, width,
...
void MPPIPlanner::GUI(mjUI& ui) {
  mjuiDef defMPPI[] = {{mjITEM_SLIDERINT, "Rollouts", 2, &num_trajectory_, "0 1"},
  ...
  mju::sprintf_arr(defMPPI[2].other, "%i %i", MinSamplingSplinePoints, MaxSamplingSplinePoints);
...
void MPPIPlanner::Plots(mjvFigure* fig_planner, mjvFigure* fig_timer, int planner_shift, int timer_shift,
