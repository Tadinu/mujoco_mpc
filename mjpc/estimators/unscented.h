...
class Unscented : public Estimator {
 public:
  // constructor
  Unscented() = default;
  Unscented(const mjModel* model_) {
    Initialize(model_);
    Reset();
  }
  ...
  void Initialize(const mjModel* model_) override {
    ...
    // settings
    settings.alpha = GetNumberOrDefault(1.0, model_, "unscented_alpha");
    settings.beta = GetNumberOrDefault(2.0, model_, "unscented_beta");
    ...
    // number of sensors
    nsensor_ =
        GetNumberOrDefault(model->nsensor, model_, "estimator_number_sensor");
  ...
  // set time
  void SetTime(double in_time) override {
    this->time = in_time;
  }
