...
class Batch : public Direct, public Estimator {
 public:
  ...
  Batch(const mjModel* model_, int length = 3, int max_history = kMaxFilterHistory)
      : Direct::Direct(model_, length, max_history) {
    // initialize memory
    Initialize(model_);
  ...
  // update
  void Update(const double* ctrl, const double* sensor, int mode) override {
    // start timer
  ...
  // set state
  void SetState(const double* state_) override {
    // state
    mju_copy(this->state.data(), state_, ndstate_);
    ...
    // q1
    configuration.Set(state_, t);

    // q0
    double* q0 = configuration.Get(t - 1);
    mju_copy(q0, state_, nq);
    mj_integratePos(model, q0, state_ + nq, -1.0 * model->opt.timestep);
  }

  // set time
  void SetTime(double time_) override {
    // copy
    double time_copy = time_;
  ...
  // set covariance
  void SetCovariance(const double* covariance_) override {
    mju_copy(this->covariance.data(), covariance_, ndstate_ * ndstate_);
  ...
  // set prior weights
  void SetPriorWeights(const double* weights, double scale) {
    // dimension
    int nv = model->nv;
