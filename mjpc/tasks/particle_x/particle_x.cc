...
#pragma omp parallel for ifMJPC_OPENMP_ENABLED
...
#else
      const auto distance_i =
        mju_rayGeom(obstacle_statesX[i].pos_.data(), nullptr, obstacle_i_size, start, ray, mjGEOM_SPHERE, nullptr);
#endif
  ...
  void ParticleX::ResidualFn::Residual(const mjModel* model, const mjData* data, double* residual) const
  {
  ...
  void ParticleX::TransitionLocked(mjModel* model, mjData* data)
  {
    ...
    SetGoalPos((double[]){
      amplitude * mju_sin(angular_vel * data->time),
      amplitude * mju_cos(angular_vel * data->time / mjPI), 0.01
    });
    MoveObstacles();
  }

  void ParticleX::ModifyScene(const mjModel* model, const mjData* data, mjvScene* scene) const
  {
  ...
  FabPlannerConfigPtr ParticleX::GetFabricsConfig() const
  {
    ...
    const FabConfigFunc fAttractor_metric = [this](const CaSX& x, const CaSX& xdot, const std::string& affix)
    {
      ...
      return FabConfigExprMeta{
        ((alpha - beta) * CaSX::exp(-1 * CaSX::pow(0.75 * x_norm, 2)) + beta) *
    ...
    // [GetFabricsConfig()] definitions for [Particle] & [ParticleXFixed]
  ...
  // PARTICLE FIXED --
  //
  std::string ParticleXFixed::Name() const { return "ParticleXFixed"; }

  void ParticleXFixed::TransitionLocked(mjModel* model, mjData* data)
  {
  ...
  void ParticleXFixed::FixedResidualFn::Residual(const mjModel* model, const mjData* data,
