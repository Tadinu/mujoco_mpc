...
std::string CoBring::XmlPath() const {
  return GetModelPath("bimanual/cobring/task.xml");
}

std::string CoBring::Name() const { return "Bimanual Cobring"; }

void CoBring::ResidualFn::Residual(const mjModel* model, const mjData* data,
                                   double* residual) const {
  int counter = 0;

  // reach left, encourage proper alignment
  double* box_to_panda0_gripper = SensorByName(model, data, "box_to_panda0_gripper");
  mju_copy3(residual + counter, box_to_panda0_gripper);
  ...
  double* box_to_panda1_gripper = SensorByName(model, data, "box_to_panda1_gripper");
  mju_copy3(residual + counter, box_to_panda1_gripper);
  residual[counter + 1] *= 2;
  residual[counter + 2] *= 2;
  counter += 3;
...
void CoBring::TransitionLocked(mjModel* model, mjData* data) {
