...
inline std::tuple<int, int, double, double> ComputeInterpolationValues(double index,
                                                                int max_index) {
...
inline int MotionLength(int id) { return kMotionLengths[id]; }

// return starting keyframe index for motion
inline int MotionStartIndex(int id) {
  int start = 0;
  for (int i = 0; i < id; i++) {
...
class Tracking : public Task {
 public:
  ...
  inline std::string Name() const override { return "Humanoid Track"; }
  ...
  // ---------------------------------------------------------------------------
  inline void TransitionLocked(mjModel *model, mjData *d) override {
    // get motion start index
