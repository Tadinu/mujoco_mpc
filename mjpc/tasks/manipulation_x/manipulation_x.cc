...
std::string manipulation_x::Bring::XmlPath() const {
  return GetModelPath("manipulation_x/task_panda_bring.xml");
}
...
void manipulation_x::Bring::TransitionLocked(mjModel* model, mjData* data) {
...
void manipulation_x::Bring::MoveObstacles() {
...
void manipulation_x::Bring::ResetLocked(const mjModel* model) {
  residual_.model_vals_ = ModelValues::FromModel(model);
}

FabPlannerConfigPtr manipulation_x::Bring::GetFabricsConfig() const {
