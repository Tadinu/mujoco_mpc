...
namespace mjpc::manipulation_x
{
  namespace
  {
    ...
    GraspState GetGraspState(const mjModel* m, const mjData* d, const ModelValues& model_vals, int body_id)
    {
      // cast a ray from one finger to the other to decide if the right body is in
      // the gripper
      const double* left_finger = d->geom_xpos + 3 * model_vals.left_finger_pad_geom_id;
      const double* right_finger = d->geom_xpos + 3 * model_vals.right_finger_pad_geom_id;
      double direction[3];
      ...
      int ray_geom_id;
      double ray_distance = mj_ray(m, d, right_finger, direction, geomgroup, 0, -1, &ray_geom_id, nullptr);
      ...
      double ray_distance2 = mj_ray(m, d, left_finger, direction, geomgroup, 0, -1, &ray_geom_id, nullptr);
      ...
      return {
        .correct_object = true,
        .gripper_object_distance = (ray_distance + ray_distance2) / finger_distance,
      };
    }

    double GripperCost(const mjModel* m, const mjData* d, const ModelValues& model_vals, bool open)
    {
      double desired_ctrl = open ? model_vals.open_gripper_ctrl : model_vals.closed_gripper_ctrl;

      return (d->ctrl[model_vals.gripper_actuator] - desired_ctrl) / model_vals.gripper_ctrl_range;
    }
  } // namespace
  ...
  ModelValues ModelValues::FromModel(const mjModel* model)
  {
    ...
    for (const char* name : {"right_pad1", "right_pad2", "left_pad1", "left_pad2"})
    {
      values.gripper_pad_geoms.push_back(mj_name2id(model, mjOBJ_GEOM, name));
    }
    values.left_finger_pad_geom_id = mj_name2id(model, mjOBJ_GEOM, "left_pad2");
    values.right_finger_pad_geom_id = mj_name2id(model, mjOBJ_GEOM, "right_pad2");

    values.gripper_actuator = mj_name2id(model, mjOBJ_ACTUATOR, "fingers");
    values.open_gripper_ctrl = model->actuator_ctrlrange[2 * values.gripper_actuator];
    values.closed_gripper_ctrl = model->actuator_ctrlrange[2 * values.gripper_actuator + 1];
    values.gripper_ctrl_range = values.closed_gripper_ctrl - values.open_gripper_ctrl;
  ...
  int ComputeControlCost(const mjModel* model, const mjData* data, double* residual)
  {
    for (int i = 0; i < model->nu; i++)
    {
      residual[i] = data->actuator_force[i] / model->actuator_gainprm[i * mjNGAIN];
    }
    return model->nu;
  }

  int ComputeGripperVelocityCost(const mjModel* model, const mjData* data, const ModelValues& model_vals,
                                 const double* object_linvel, double* residual)
  {
    double gripper_linvel[6];
    mj_objectVelocity(model, data, mjOBJ_SITE, model_vals.gripper_site_id, gripper_linvel,
                      /*flg_local=*/0);
  ...
  int ComputeMaxVelocityCost(const mjModel* model, const mjData* data, const std::vector<int>& body_ids,
                             double* residual)
  {
    double max_speed = 0;
    mju_zero(residual, 6);
  ...
  // object
  double BlockedCost(const mjModel* model, const mjData* data, const ModelValues& model_vals, const double* pos,
                     const double* object, int exclude_body_id)
  {
    ...
    int ray_geom_id;
    double ray_distance = mj_ray(model, data, pos, direction, geomgroup, 0, exclude_body_id, &ray_geom_id, nullptr);
    // if there is something between the hand and the object, return 1.
    return ray_distance > -1 && ray_distance < distance;
  }

  double GraspQualityCost(const mjModel* m, const mjData* d, const ModelValues& model_vals, int body_id,
                          bool release)
  {
    if (release)
    {
  ...
  double CarefulCost(const mjModel* model, const mjData* data, const ModelValues& model_vals,
                     int object_body_id)
  {
    double result = 0;
  ...
  void ComputeRobotiqHandPos(const mjModel* model, const mjData* data, const ModelValues& model_vals,
                             double* pos)
  {
