...
static void generateRay(const float raster2camera[4][4], const float camera2world[4][4], float x, float y, rt_serial::Ray &ray) {
    ray.mint = 0.f;
    ray.maxt = 1e30f;
...
static inline bool BBoxIntersect(const float bounds[2][3], const rt_serial::Ray &ray) {
...
inline bool TriIntersect(const rt_serial::Triangle &tri, rt_serial::Ray &ray) {
...
bool BVHIntersect(const rt_serial::LinearBVHNode nodes[], const rt_serial::Triangle tris[], rt_serial::Ray &r) {
    rt_serial::Ray ray = r;
    bool hit = false;
    ...
    while (true) {
        ...
        const rt_serial::LinearBVHNode &node = nodes[nodeNum];
...
class ISPCRayTracing {
public:
    static void raytrace_serial(int width, int height, int baseWidth, int baseHeight, const float raster2camera[4][4],
                                const float camera2world[4][4], float image[], int id[], const rt_serial::LinearBVHNode nodes[],
                                const rt_serial::Triangle triangles[]) {
