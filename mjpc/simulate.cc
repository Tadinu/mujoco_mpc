...
const mjuiDef defFile[] = {
    {mjITEM_SECTION, "File", 0, nullptr, "AF"},      {mjITEM_BUTTON, "Save xml", 2, nullptr, ""},
...
void UpdateProfiler(mj::Simulate* sim) {
  ...
  float tdata[5] = {static_cast<float>(total / number),
                    ...
                    static_cast<float>(sim->d->timer[mjTIMER_CONSTRAINT].duration / number), 0};
  ...
  float sdata[6] = {static_cast<float>(sim->m->nv),   static_cast<float>(sim->m->nbody),
...
// prepare info text
void UpdateInfoText(mj::Simulate* sim, char (&title)[mj::Simulate::kMaxFilenameLength],
                    char (&content)[mj::Simulate::kMaxFilenameLength], double interval) {
  mjModel* m = sim->m;
  mjData* d = sim->d;
...
void MakeRenderingSection(mj::Simulate* sim, int oldstate) {
  ...
  // create tree slider
  mjuiDef defTree[] = {{mjITEM_SLIDERINT, "Tree depth", 2, &sim->opt.bvh_depth, "0 20"}, {mjITEM_END}};
  ...
  for (int i = 0; i < sim->m->njnt && itemcnt < mjMAXUIITEM; i++)
    if ((sim->m->jnt_type[i] == mjJNT_HINGE || sim->m->jnt_type[i] == mjJNT_SLIDE)) {
      ...
      if (sim->m->jnt_limited[i])
        mju::sprintf_arr(defSlider[0].other, "%.4g %.4g", sim->m->jnt_range[2 * i],
...
void MakeControlSection(mj::Simulate* sim, int oldstate) {
  ...
  for (int i = 0; i < sim->m->nu && itemcnt < mjMAXUIITEM; i++) {
    ...
    if (sim->m->actuator_ctrllimited[i])
      mju::sprintf_arr(defSlider[0].other, "%.4g %.4g", sim->m->actuator_ctrlrange[2 * i],
...
void UiEvent(mjuiState* state) {
  ...
  if (state->type == mjEVENT_PRESS && state->mouserect == 3 && m) {
    ...
    if (state->doubleclick) {
      ...
      int selbody =
          mjv_select(m, d, &sim->opt, static_cast<mjtNum>(r.width) / r.height, (state->x - r.left) / r.width,
                     (state->y - r.bottom) / r.height, &sim->scn, selpnt, &selgeom, &selflex, &selskin);
  ...
  if (state->type == mjEVENT_MOVE && state->dragrect == 3 && m) {
    ...
    mjrRect r = state->rect[3];
    if (sim->pert.active)
      mjv_movePerturb(m, d, action, state->dx / r.height, -state->dy / r.height, &sim->scn, &sim->pert);
    else
      mjv_moveCamera(m, action, state->dx / r.height, -state->dy / r.height, &sim->scn, &sim->cam);
...
Simulate::Simulate(std::unique_ptr<PlatformUIAdapter> platform_ui, std::shared_ptr<mjpc::Agent> a)
    : platform_ui(std::move(platform_ui)), uistate(this->platform_ui->state()), agent(std::move(a)) {}
...
void Simulate::Load(mjModel* m, mjData* d, std::string displayed_filename, bool delete_old_m_d) {
  this->mnew = m;
  this->dnew = d;
