...
Inertial Inertial::fromXml(TiXmlElement *xml) {
  ...
  if (mass_xml != nullptr) {
    if (mass_xml->Attribute("value") != nullptr) {
      try {
        i.mass = absl::any_cast<double>(mass_xml->Attribute("value"));
  ...
  if (inertia_xml != nullptr) {
    if (inertia_xml->Attribute("ixx") && inertia_xml->Attribute("ixy") && inertia_xml->Attribute("ixz") &&
      ...
      try {
        i.ixx = absl::any_cast<double>(inertia_xml->Attribute("ixx"));
        i.ixy = absl::any_cast<double>(inertia_xml->Attribute("ixy"));
        i.ixz = absl::any_cast<double>(inertia_xml->Attribute("ixz"));
        i.iyy = absl::any_cast<double>(inertia_xml->Attribute("iyy"));
        i.iyz = absl::any_cast<double>(inertia_xml->Attribute("iyz"));
        i.izz = absl::any_cast<double>(inertia_xml->Attribute("izz"));
      } catch (const absl::bad_any_cast &e) {
        std::ostringstream error_msg;
        error_msg << "Error while parsing link '" << getParentLinkName(xml)
                  << "Inertial: one of the inertia elements is not a valid double:" << " ixx ["
                  << inertia_xml->Attribute("ixx") << "]" << " ixy [" << inertia_xml->Attribute("ixy") << "]"
                  << " ixz [" << inertia_xml->Attribute("ixz") << "]" << " iyy ["
                  << inertia_xml->Attribute("iyy") << "]" << " iyz [" << inertia_xml->Attribute("iyz") << "]"
