  ...
  { \
    ...
    if (!val_str.empty()) { \
      if (!absl::SimpleAtod(val_str, &out)) { \
        ...
        error_msg << "Error while parsing joint dynamics'" << getParentJointName(xml) << "': "+ std::string(attr) +" (" \
            << val_str << ") is not a double!"; \
...
#define VERIFY_JOINT_LIMITS(attr, out) \
  { \
    ...
    if (!val_str.empty()) { \
      if (!absl::SimpleAtod(val_str, &out)) { \
        std::ostringstream error_msg; \
        error_msg << "Error while parsing joint limits '" << getParentJointName(xml) << "': "+ std::string(attr) +" (" \
            << val_str << ") is not a double"; \
        throw URDFParseError(error_msg.str()); \
      } \
    } \
