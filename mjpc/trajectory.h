...
static constexpr int kMaxTrajectoryHorizon = 512;
// maximum return value
static constexpr double kMaxReturnValue = 1.0e6;
...
class Trajectory {
 public:
  ...
  void Initialize(int dim_state_, int dim_action_, int dim_residual_,
                  int num_trace_, int horizon_) {
    this->horizon = horizon_;
    this->dim_state = dim_state_;
    this->dim_action = dim_action_;
    this->dim_residual = dim_residual_;
    this->dim_trace = 3 * num_trace_;
    this->failure = false;
    }
